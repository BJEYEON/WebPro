...
<%
	...
	if(MemberDao.EXISTENT == dao.midConfirm("ccc")){
		out.println("ccc는 중복된 ID");
	}else if(MemberDao.NONEXISTENT == dao.midConfirm("ccc")){
		out.println("ccc는 사용가능한 ID");
		out.println("<h3>2.회원가입</h3>");
		int result = dao.joinMember(new MemberDto("ccc","1","홍길","h@h.com","NOIMG.JPG",null, "경기",null));
	...
	MemberDto member = dao.getMember("ccc");
	out.println("<p>"+member+"</p>");
	out.println("<h1>5. 회원정보 수정(aaa회원정보 수정)</h1>");
	mid = "ccc";
