...
<body>
	...
	<c:if test="${deleteResult eq SUCCESS}">
		<script>alert('${param.bid}번 글 삭제성공');</script>
	</c:if>
	<c:if test="${deleteResult eq FAIL}">
		<script>alert('${param.bid}번 글 삭제실패'); history.back();</script>
	</c:if>
	...
	<table>
		...
		<c:set var="oNum" value="${paging.totCnt - paging.startRow+1 }"/>
	...
	<div class="paging">
		<c:if test="${paging.startPage > BLOCKSIZE }">
